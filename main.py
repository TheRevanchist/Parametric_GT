...
def main():
    ...
    out_dir = os.path.join(root, 'out', current_dataset, 'feature_data_test')
    batch_size = 1
    ...
    for j in xrange(1, 5):
        ...
        for i, net_type in enumerate(list_of_net_names):
            ...
            train_loader = prepare_loader_val(dataset_test, stats, batch_size)
            # if net is densenet
            if net_type[:3] == 'den':
                fc7_features_tr, labels, net, fnames = fe.extract_features_train(net, feature_size, dataset_size_test,
